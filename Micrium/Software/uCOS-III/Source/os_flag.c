                      ...
                      OS_ERR       *p_err)
{
    ...
    switch (mode) {
        case OS_OPT_PEND_FLAG_SET_ALL:                      /* See if all required flags are set                      */
             ...
             if (flags_rdy == flags) {                      /* Must match ALL the bits that we want                   */
                 ...
                 OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsRdy = flags_rdy;         /* Save flags that were ready                             */
                 if (p_ts != (CPU_TS *)0) {
                    *p_ts  = p_grp->TS;
                 }
    ...
    flags_rdy = OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsRdy;
    if (consume == DEF_TRUE) {                              /* See if we need to consume the flags                    */
        switch (mode) {
            case OS_OPT_PEND_FLAG_SET_ALL:
...
OS_FLAGS  OSFlagPendGetFlagsRdy (OS_ERR  *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    flags = OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsRdy;
    CPU_CRITICAL_EXIT();
   *p_err = OS_ERR_NONE;
    return (flags);
}
...
void  OS_FlagBlock (OS_PEND_DATA  *p_pend_data,
                    ...
                    OS_TICK        timeout)
{
    OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsPend = flags;                         /* Save the flags that we need to wait for                */
    OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsOpt  = opt;                           /* Save the type of wait we are doing                     */
    OSTCBCurPtr[GetCurrentProcessorNumber()]->FlagsRdy  = (OS_FLAGS)0;
