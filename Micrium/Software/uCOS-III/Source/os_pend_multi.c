                        ...
                        OS_TICK        timeout)
{
    ...
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendOn             = OS_TASK_PEND_ON_MULTI;   /* Resource not available, wait until it is            */
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendStatus         = OS_STATUS_PEND_OK;
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendDataTblEntries = tbl_size;
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendDataTblPtr     = p_pend_data_tbl;

    OS_TaskBlock(OSTCBCurPtr[GetCurrentProcessorNumber()],                                  /* Block the task waiting for object to be posted ...  */
                 timeout);                                     /* ... but with a timeout if not                       */

    for (i = 0u; i < tbl_size; i++) {
        p_pend_data_tbl->TCBPtr = OSTCBCurPtr[GetCurrentProcessorNumber()];                 /* Every entry points back to the TCB of the task      */
