				 ...
				 DWORD OSCurCPUNum)
{
    ...
    OS_CRITICAL_ENTER();
    OS_TickListInsertDly(OSTCBCurPtr[GetCurrentProcessorNumber()],
                         dly,
                         opt,
                         p_err);
...
#endif
    OS_RdyListRemove(OSTCBCurPtr[GetCurrentProcessorNumber()]);                          /* Remove current task from ready list                    */
    OS_CRITICAL_EXIT_NO_SCHED();
    OSSched(OSCurCPUNum);                                              /* Find next task to run!                                 */
   *p_err = OS_ERR_NONE;
}
...
#endif
        OS_RdyListRemove(OSTCBCurPtr[GetCurrentProcessorNumber()]);                      /* Remove current task from ready list                    */
		for (int curCPU = 0;curCPU < 4;curCPU++)
		{
			if (CPU_STATS[curCPU]->CPU_CurTask_TCB == OSTCBCurPtr[GetCurrentProcessorNumber()])
			{
				switch (curCPU)
				{
				case 0:
                       ...
                       OS_ERR  *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    if (p_tcb == OSTCBCurPtr[GetCurrentProcessorNumber()]) {                             /* Not possible for the running task to be delayed!       */
       *p_err = OS_ERR_TASK_NOT_DLY;
        CPU_CRITICAL_EXIT();
        return;
    }
