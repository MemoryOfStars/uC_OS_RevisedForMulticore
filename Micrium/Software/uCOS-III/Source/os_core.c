...
void  OSInit (OS_ERR  *p_err)
{
    ...
    OSTCBCurPtr[0]                     = (OS_TCB *)0;          /* Initialize OS_TCB pointers to a known state            */
	OSTCBCurPtr[1]					   = (OS_TCB *)0;
	OSTCBCurPtr[2]					   = (OS_TCB *)0;
	OSTCBCurPtr[3]					   = (OS_TCB *)0;
	...
	OS_Init_CPU_Stat();										/*初始化描述CPU状态的结构*/
...
void  OSIntExit (void)
{
    ...
    if (OSTCBHighRdyPtr == OSTCBCurPtr[GetCurrentProcessorNumber()]) {                   /* Current task still the highest priority?               */
        CPU_INT_EN();                                       /* Yes                                                    */
        return;
    }
...
************************************************************************************************************************
*/
//OSCurCPUNum是要上下文切换的目标任务所在的CPU号
void  OSSched (DWORD OSCurCPUNum)
{
    CPU_SR_ALLOC();
	...
	{
	case 0:
		CPU_STATS[OSCurCPUNum]->CPU_CurTask_TCB = &OSIdleTaskTCB0;
		(&OSIdleTaskTCB0)->TaskState = 2;
		break;
	case 1:CPU_STATS[OSCurCPUNum]->CPU_CurTask_TCB = &OSIdleTaskTCB1;
		(&OSIdleTaskTCB1)->TaskState = 2;
		break;
	case 2:CPU_STATS[OSCurCPUNum]->CPU_CurTask_TCB = &OSIdleTaskTCB2;
		(&OSIdleTaskTCB2)->TaskState = 2;
		break;
	case 3:CPU_STATS[OSCurCPUNum]->CPU_CurTask_TCB = &OSIdleTaskTCB3;
		(&OSIdleTaskTCB3)->TaskState = 2;
		break;
	}

	//CPU_STATS[OSCurCPUNum]->CPU_CurTask_TCB = &OSIdleTaskTCB;
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* ISRs still nested?                                     */
    ...
    if (OSTCBHighRdyPtr == OSTCBCurPtr[GetCurrentProcessorNumber()]) {                   /* Current task is still highest priority task?           */
    ...
    OS_TASK_SW(OSCurCPUNum);                                           /* Perform a task level context switch                    */
    CPU_INT_EN();
...
void  OSSchedUnlock (OS_ERR  *p_err,DWORD OSCurCPUNum)
{
    CPU_SR_ALLOC();
    ...
    OSSched(OSCurCPUNum);                                              /* Run the scheduler                                      */
   *p_err = OS_ERR_NONE;
}
...
void  OSStart (OS_ERR  *p_err)
{
    ...
    if (OSRunning == OS_STATE_OS_STOPPED) {
        ...
        OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;
        OSTCBCurPtr[0]  = OSTCBHighRdyPtr;					/*第一个任务在0号CPU运行*/
...
void  OS_IdleTaskInit (OS_ERR  *p_err)
{
    ...
    OSTaskCreate((OS_TCB     *)&OSIdleTaskTCB0,
                 (CPU_CHAR   *)((void *)"uC/OS-III Idle Task0"),
                 (OS_TASK_PTR)OS_IdleTask,
                 (void       *)0,
                 (OS_PRIO     )(OS_CFG_PRIO_MAX - 1u),
                 (CPU_STK    *)OSCfg_IdleTaskStkBasePtr0,
                 ...
                 (OS_ERR     *)p_err);
	OSTaskCreate((OS_TCB     *)&OSIdleTaskTCB1,
		...
		(OS_OPT)(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR | OS_OPT_TASK_NO_TLS),
		(OS_ERR     *)p_err);
	OSTaskCreate((OS_TCB     *)&OSIdleTaskTCB2,
		...
		(OS_OPT)(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR | OS_OPT_TASK_NO_TLS),
		(OS_ERR     *)p_err);
	OSTaskCreate((OS_TCB     *)&OSIdleTaskTCB3,
		(CPU_CHAR   *)((void *)"uC/OS-III Idle Task3"),
		(OS_TASK_PTR)OS_IdleTask,
		(void       *)0,
		(OS_PRIO)(OS_CFG_PRIO_MAX - 1u),
		(CPU_STK    *)OSCfg_IdleTaskStkBasePtr3,
		(CPU_STK_SIZE)OSCfg_IdleTaskStkLimit,
		(CPU_STK_SIZE)OSCfg_IdleTaskStkSize,
		(OS_MSG_QTY)0u,
		(OS_TICK)0u,
               ...
               OS_TICK        timeout)
{
    ...
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendOn     = pending_on;                    /* Resource not available, wait until it is              */
    OSTCBCurPtr[GetCurrentProcessorNumber()]->PendStatus = OS_STATUS_PEND_OK;

    OS_TaskBlock(OSTCBCurPtr[GetCurrentProcessorNumber()],                                /* Block the task and add it to the tick list if needed  */
                 timeout);
    ...
    if (p_obj != (OS_PEND_OBJ *)0) {                         /* Add the current task to the pend list ...             */
        ...
        OS_PendDataInit((OS_TCB       *)OSTCBCurPtr[GetCurrentProcessorNumber()],         /* Initialize the remaining field                        */
                        (OS_PEND_DATA *)p_pend_data,
                        (OS_OBJ_QTY    )1);
        OS_PendListInsertPrio(p_pend_list,                   /* Insert in the pend list in priority order             */
                              p_pend_data);
    } else {
        OSTCBCurPtr[GetCurrentProcessorNumber()]->PendDataTblEntries = (OS_OBJ_QTY    )0; /* If no object being pended on the clear these fields   */
        OSTCBCurPtr[GetCurrentProcessorNumber()]->PendDataTblPtr     = (OS_PEND_DATA *)0; /* ... in the TCB                                        */
    }
#if OS_CFG_DBG_EN > 0u
    OS_PendDbgNameAdd(p_obj,
                      OSTCBCurPtr[GetCurrentProcessorNumber()]);
#endif
}
...
#endif
    OSTaskCtxSwCtr++;                                       /* Increment context switch counter                       */
    OS_TASK_SW(0);                                           /* Perform a task level context switch                    */
    CPU_INT_EN();
}
#endif
