                        ...
                        OS_ERR   *p_err)
{
    ...
    if (p_tcb == (OS_TCB *)0) {                             /* Are we changing the priority of 'self'?                */
        CPU_CRITICAL_ENTER();
        p_tcb = OSTCBCurPtr[GetCurrentProcessorNumber()];
        CPU_CRITICAL_EXIT();
    }
    ...
    OSSched(GetCurrentProcessorNumber());                                              /* Run highest priority task ready                        */
                    ...
                    OS_ERR        *p_err)
{
   ...
   *p_err = OS_ERR_NONE;
    ...
    OS_CRITICAL_EXIT_NO_SCHED();
	int i0 = GetCurrentProcessorNumber();
    OSSched(GetCurrentProcessorNumber());
}
                 ...
                 OS_ERR  *p_err)
{
    ...
    if (p_tcb == &OSIdleTaskTCB0 || p_tcb != &OSIdleTaskTCB1 || p_tcb != &OSIdleTaskTCB2 || p_tcb != &OSIdleTaskTCB3) {                          /* Not allowed to delete the idle task                    */
    ...
    if (p_tcb == (OS_TCB *)0) {                             /* Delete 'Self'?                                         */
        CPU_CRITICAL_ENTER();
        p_tcb  = OSTCBCurPtr[GetCurrentProcessorNumber()];                               /* Yes.                                                   */
        CPU_CRITICAL_EXIT();
    }
   ...
   *p_err = OS_ERR_NONE;                                    /* See Note #1.                                           */
   int i = GetCurrentProcessorNumber();
    OSSched(GetCurrentProcessorNumber());                                              /* Find new highest priority task                         */
}
#endif
                    ...
                    OS_ERR       *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    p_msg_q = &OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQ;                           /* Any message waiting in the message queue?              */
    ...
    if (*p_err == OS_ERR_NONE) {
        ...
        if (p_ts != (CPU_TS *)0) {
            OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime = OS_TS_GET() - *p_ts;
            if (OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTimeMax < OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime) {
                OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTimeMax = OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime;
            }
        }
#endif
        CPU_CRITICAL_EXIT();
...
#endif
	int i = GetCurrentProcessorNumber();
    OSSched(GetCurrentProcessorNumber());                                              /* Find the next highest priority task ready to run       */

    CPU_CRITICAL_ENTER();
    switch (OSTCBCurPtr[GetCurrentProcessorNumber()]->PendStatus) {
        case OS_STATUS_PEND_OK:                             /* Extract message from TCB (Put there by Post)           */
             p_void      = OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgPtr;
            *p_msg_size  = OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgSize;
             if (p_ts != (CPU_TS *)0) {
                *p_ts  = OSTCBCurPtr[GetCurrentProcessorNumber()]->TS;
#if OS_CFG_TASK_PROFILE_EN > 0u
                OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime = OS_TS_GET() - OSTCBCurPtr[GetCurrentProcessorNumber()]->TS;
                if (OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTimeMax < OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime) {
                    OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTimeMax = OSTCBCurPtr[GetCurrentProcessorNumber()]->MsgQPendTime;
                }
#endif
                               ...
                               OS_ERR  *p_err)
{
    ...
    if ((opt & OS_OPT_POST_NO_SCHED) == (OS_OPT)0) {
		int LowestCPUNum = OSTCBCurPtr[0]->Prio;
		int flag = 0;
		for (int i = 0;i < 4;i++)
		{
			if (LowestCPUNum > OSTCBCurPtr[i]->Prio)
			{
				flag = i;
				LowestCPUNum = OSTCBCurPtr[i]->Prio;
			}
		}													/*找出最低优先级任务的CPU号*/
        OSSched(flag);                                          /* Run the scheduler                                      */
    }
   *p_err = OS_ERR_NONE;
    return (DEF_TRUE);
}
						   ...
						   DWORD     OSCurCPUNum)
{
    ...
    if (p_err == (OS_ERR *)0) {
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
        TRACE_OS_TASK_SEM_PEND_FAILED(OSTCBCurPtr[GetCurrentProcessorNumber()]);         /* Record the event.                                      */
#endif
        OS_SAFETY_CRITICAL_EXCEPTION();
        return ((OS_SEM_CTR)0);
    }
    ...
    CPU_CRITICAL_ENTER();
    if (OSTCBCurPtr[OSCurCPUNum]->SemCtr > (OS_SEM_CTR)0) {              /* See if task already been signaled                      */
        OSTCBCurPtr[OSCurCPUNum]->SemCtr--;
        ctr    = OSTCBCurPtr[OSCurCPUNum]->SemCtr;
        if (p_ts != (CPU_TS *)0) {
           *p_ts  = OSTCBCurPtr[OSCurCPUNum]->TS;
        }
#if OS_CFG_TASK_PROFILE_EN > 0u
        OSTCBCurPtr[OSCurCPUNum]->SemPendTime = OS_TS_GET() - OSTCBCurPtr[OSCurCPUNum]->TS;
        if (OSTCBCurPtr[OSCurCPUNum]->SemPendTimeMax < OSTCBCurPtr[OSCurCPUNum]->SemPendTime) {
            OSTCBCurPtr[OSCurCPUNum]->SemPendTimeMax = OSTCBCurPtr[OSCurCPUNum]->SemPendTime;
        }
#endif
        CPU_CRITICAL_EXIT();
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
        TRACE_OS_TASK_SEM_PEND(OSTCBCurPtr[GetCurrentProcessorNumber()]);                /* Record the event.                                      */
#endif
       *p_err = OS_ERR_NONE;
        return (ctr);
    }
    ...
    OS_CRITICAL_EXIT_NO_SCHED();
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
    TRACE_OS_TASK_SEM_PEND_BLOCK(OSTCBCurPtr[GetCurrentProcessorNumber()]);              /* Record the event.                                      */
#endif
    OSSched(OSCurCPUNum);                                              /* Find next highest priority task ready to run           */

    CPU_CRITICAL_ENTER();
    switch (OSTCBCurPtr[OSCurCPUNum]->PendStatus) {                      /* See if we timed-out or aborted                         */
...
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
             TRACE_OS_TASK_SEM_PEND(OSTCBCurPtr[OSCurCPUNum]);           /* Record the event.                                      */
#endif
            *p_err = OS_ERR_NONE;
             break;
...
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
             TRACE_OS_TASK_SEM_PEND_FAILED(OSTCBCurPtr[OSCurCPUNum]);    /* Record the event.                                      */
#endif
                      ...
                      OS_ERR  *p_err)
{
    ...
    if (p_tcb == &OSIdleTaskTCB0|| p_tcb != &OSIdleTaskTCB1|| p_tcb != &OSIdleTaskTCB2|| p_tcb != &OSIdleTaskTCB3) {                          /* Make sure not suspending the idle task                 */
...
void  OS_TaskReturn (void)
{
    ...
    OSTaskReturnHook(OSTCBCurPtr[GetCurrentProcessorNumber()]);                          /* Call hook to let user decide on what to do             */
#if OS_CFG_TASK_DEL_EN > 0u
        ...
        case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
             if (p_tcb->PendOn == OS_TASK_PEND_ON_TASK_SEM) {   /* Is task signaled waiting for a signal?             */
                 ...
                 if ((opt & OS_OPT_POST_NO_SCHED) == (OS_OPT)0) {
					 ...
					 for (int i = 0;i < 4;i++)
					 {
						 if (LowestCPUNum > CPU_STATS[i]->CPU_CurTask_TCB->TaskState)
						 {
							 flag = i;
							 LowestCPUNum = CPU_STATS[i]->CPU_CurTask_TCB->TaskState;
						 }
					 }													/*找出最低优先级任务的CPU号*/
					 OSSched(flag);                /* Run the scheduler                                  */
                      ...
                      OS_ERR  *p_err)
{
    ...
    if (p_tcb == OSTCBCurPtr[GetCurrentProcessorNumber()]) {
