                   ...
                   OS_ERR    *p_err)
{
    ...
    if (p_mutex->OwnerNestingCtr == (OS_NESTING_CTR)0) {    /* Resource available?                                    */
        p_mutex->OwnerTCBPtr       =  OSTCBCurPtr[GetCurrentProcessorNumber()];          /* Yes, caller may proceed                                */
        p_mutex->OwnerNestingCtr   = (OS_NESTING_CTR)1;
        if (p_ts != (CPU_TS *)0) {
           *p_ts  = p_mutex->TS;
        }
        OS_MutexGrpAdd(OSTCBCurPtr[GetCurrentProcessorNumber()], p_mutex);               /* Add mutex to owner's group                             */
        CPU_CRITICAL_EXIT();
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
    ...
    if (OSTCBCurPtr[GetCurrentProcessorNumber()] == p_mutex->OwnerTCBPtr) {              /* See if current task is already the owner of the mutex  */
    ...
    if (p_tcb->Prio > OSTCBCurPtr[GetCurrentProcessorNumber()]->Prio) {                  /* See if mutex owner has a lower priority than current   */
        OS_TaskChangePrio(p_tcb, OSTCBCurPtr[GetCurrentProcessorNumber()]->Prio);
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
                   ...
                   OS_ERR    *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    if (OSTCBCurPtr[GetCurrentProcessorNumber()] != p_mutex->OwnerTCBPtr) {              /* Make sure the mutex owner is releasing the mutex       */
        CPU_CRITICAL_EXIT();
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
        TRACE_OS_MUTEX_POST_FAILED(p_mutex);                /* Record the event.                                      */
#endif
    ...
    OS_MutexGrpRemove(OSTCBCurPtr[GetCurrentProcessorNumber()], p_mutex);                /* Remove mutex from owner's group                        */
    ...
    if (p_pend_list->NbrEntries == (OS_OBJ_QTY)0) {         /* Any task waiting on mutex?                             */
       ...
       *p_err = OS_ERR_NONE;
        return;
                                                            ...
                                                            /* Yes                                                    */
    if (OSTCBCurPtr[GetCurrentProcessorNumber()]->Prio != OSTCBCurPtr[GetCurrentProcessorNumber()]->BasePrio) {       /* Has owner inherited a priority?                        */
        prio_new = OS_MutexGrpPrioFindHighest(OSTCBCurPtr[GetCurrentProcessorNumber()]); /* Yes, find highest priority pending                     */
        prio_new = prio_new > OSTCBCurPtr[GetCurrentProcessorNumber()]->BasePrio ? OSTCBCurPtr[GetCurrentProcessorNumber()]->BasePrio : prio_new;
        if (prio_new > OSTCBCurPtr[GetCurrentProcessorNumber()]->Prio) {
            OS_RdyListRemove(OSTCBCurPtr[GetCurrentProcessorNumber()]);
            OSTCBCurPtr[GetCurrentProcessorNumber()]->Prio = prio_new;                   /* Lower owner's priority back to its original one        */
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN > 0u))
            TRACE_OS_MUTEX_TASK_PRIO_DISINHERIT(OSTCBCurPtr[GetCurrentProcessorNumber()], prio_new);
#endif
            OS_PrioInsert(prio_new);
            OS_RdyListInsertTail(OSTCBCurPtr[GetCurrentProcessorNumber()]);              /* Insert owner in ready list at new priority             */
            OSPrioCur         = prio_new;
        }
    }
